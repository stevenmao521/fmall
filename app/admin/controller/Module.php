...
class Module extends Common
{
    ...
    public function edit(){
        if(request()->isPost()){
            ...
            $info = $this->dao->field('id,title,name,olist,description,listfields')->where($map)->find();
        ...
        if(request()->isPost()){
            ...
            if($emptytable=='0'){
			  ...
			  PRIMARY KEY (`id`)
                ...
                $fields = "(id,moduleid,field,name,tips,required,minlength,maxlength,pattern,errormsg,class,type,setup,ispost,unpostgroup,listorder,status,issystem,isedit)";
                Db::execute("INSERT INTO `".$prefix."field` {$fields} VALUES ('', '".$moduleid."', 'createtime', '创建时间', '', '1', '0', '0', 'date', '', '', 'datetime', '','1','',  '97', '1', '1', '0')");
            }
            if ($moduleid  !==false) {
                savecache('Module');
                $result['code'] = 1;
    ...
    public function editStatus(){
        ...
        //判断当前状态情况
        $field = db('field');
        $status=$field->where($map)->value('isedit');
        if($status==1){
            $data['isedit'] = 0;
        }else{
            $data['isedit'] = 1;
    ...
    public function hideStatus(){
        ...
        //判断当前状态情况
        $field = db('field');
        $status=$field->where($map)->value('ishide');
        if($status==1){
            $data['ishide'] = 0;
        }else{
            $data['ishide'] = 1;
