...
namespace clt;
use clt\Leftnav;
use app\xq\controller\Acter as acter; #事件类
...
class Form{
    ...
    public function fkey($info,$value){
        ...
        $actname = $info['setup']['action'];#绑定事件
        ...
        if ($modname == 'office' && $module != 'agency') {
            ...
            $rule = rtrim($rule,',');
            if ($rule) {
                $list = db($modname)->where("id in ({$rule})")->where('istrash=0')->field($fields)->select();
            } else {
                $list = array();
            }
        } else {
            $list = db($modname)->field($fields)->where('istrash=0')->select();
        }
        if ($info['required']) {
            $parseStr = '<select lay-search="" id="'.$id.'" name="'.$field.'"  '.$validate.' lay-filter="'.$field.'">';
        } else {
            $parseStr = '<select lay-search="" id="'.$id.'" name="'.$field.'"  '.$validate.' lay-filter="'.$field.'">';
        }
        ...
        #当前下拉所绑定事件
        if ($actname) {
            $parseStr .= $this->acter->$actname($info);
        }
        return $parseStr;
    }
    
    #联动
    public function rekey($info,$value){
        $info['setup']=is_array($info['setup']) ? $info['setup'] : string2array($info['setup']);
        $validate = getvalidate($info);
        $action = ACTION_NAME;
        $id = $field = $info['field'];
        ...
        $modname = $info['setup']['modname'];
        $keyname = $info['setup']['keyname'];
        $listen = $info['setup']['listen'];
        $rekey = $info['setup']['rekey'];#关联字段
        $actname = $info['setup']['action'];#绑定事件
        
        $key = mz_str_delimeter($keyname, ',');
        $str = "";
        if (is_array($key)) {
            foreach ($key as $k=>$v){
                $str .= "value.{$v}+' '+";
        ...
        if ($value) {
            ...
            if ($list) {
                foreach ($list as $k=>$v) {
                    ...
                    $html_str .= "<option value='{$v['id']}' {$selected}>{$str_edit}</option>";
                }
            }
        }
        ...
        layui.use('form', function () {
            var form = layui.form;
            form.on('select({$listen})', function(data){
            var pid = data.value;
            var loading = layer.load(1, {shade: [0.1, '#fff']});
                $.get(\"{$url}?pid=\"+pid+\"&modname={$modname}&keyname={$keyname}&rekey=$rekey\", function (data) {
                    layer.close(loading);
                    var html='<option value=\"\">请选择{$info['name']}</option>';
                    if (data) {
                        $.each(data, function (i, value) {
                            html += '<option value='+value.id+'>'+{$str}+'</option>';
                        });
                    }
                    $('#{$id}').html(html);
                    form.render()
    ...
    public function datetime_bak($info,$value){
        $info['setup']=is_array($info['setup']) ? $info['setup'] : string2array($info['setup']);
        $field = $info['field'];
        $name = $info['name'];
        $action = ACTION_NAME;
    ...
    public function datetime($info,$value){
        ...
        $value = $value ?  toDate($value,"Y-m-d H:i") : "";
